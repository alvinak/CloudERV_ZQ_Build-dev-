// DcmImageView.cpp : implementation file
//
...
struct SeriesObjectSort1Data {
	std::string key;
	std::string caption;
	int index;
};

int SeriesObjectSort1Data_Compare(const SeriesObjectSort1Data *a, const SeriesObjectSort1Data *b)
{
...
IMPLEMENT_DYNCREATE(CDcmImageView, CView)

CDcmImageView::CDcmImageView()
{
	int ni;
...
BEGIN_MESSAGE_MAP(CDcmImageView, CView)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_ERASEBKGND()
...
// CDcmImageView drawing

void CDcmImageView::OnDraw(CDC* pDC)
{
	CDcmImageDoc* pDoc = GetDocument();
	// TODO: add draw code here
}


// CDcmImageView diagnostics

#ifdef _DEBUG
void CDcmImageView::AssertValid() const
{
	CView::AssertValid();
}

#ifndef _WIN32_WCE
void CDcmImageView::Dump(CDumpContext& dc) const
{
	CView::Dump(dc);
}
#endif

CDcmImageDoc* CDcmImageView::GetDocument() const // non-debug version is inline
{
	ASSERT(m_pDocument->IsKindOf(RUNTIME_CLASS(CDcmImageDoc)));
	return (CDcmImageDoc*)m_pDocument;
}
...
void CDcmImageView::ShowMessage(int nCode)
{
	CMainFrame *pMainFrame = (CMainFrame *) AfxGetMainWnd();
	pMainFrame->ShowMessageEx(nCode);
}

void CDcmImageView::ShowMessage(const TCHAR *szMsg)
{
	CMainFrame *pMainFrame = (CMainFrame *) AfxGetMainWnd();
	pMainFrame->ShowMessageEx(szMsg);
}

BOOL CDcmImageView::IsDicomViewerEmpty()
{
	BOOL bSeriesMode;
	int nCount;
...
void CDcmImageView::SetWinLevel(double fCen, double fWid)
{
	m_wndDicomViewer.SetWinLevel(fCen, fWid);
	m_wndDicomViewer.InvalidateRect(NULL);
}

void CDcmImageView::SetImgLayout(int nRows, int nCols, BOOL bCustom)
{
	BOOL bSeriesMode;
	int nIndex;
...
// CDcmImageView message handlers


int CDcmImageView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CDcmImageView::OnSize(UINT nType, int cx, int cy)
{
...
BOOL CDcmImageView::OnEraseBkgnd(CDC* pDC)
{
	return TRUE;
}

void CDcmImageView::OnInitMenuPopup(CMenu *pPopupMenu, UINT nIndex,BOOL bSysMenu)
 {
...
void CDcmImageView::OnCellWndClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	UINT nID;
...
void CDcmImageView::OnCellWndRBClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	UINT nID;
	CPoint ptCurrent;
...
void CDcmImageView::ReadIniValues()
{
...
void CDcmImageView::SetWinLevelOptions(CustomWinLevelStruct *pWinLevel)
{
	int ni;
	const TCHAR *szCustomWinLevelSection = _T("CustomWinLevel");
...
void CDcmImageView::SetSensitivity(int nValue)
{
	const TCHAR *szSettingsSection = _T("Settings");
	CWinApp *pApp = AfxGetApp();
...
void CDcmImageView::ShowOptionsPopupMenu()
{
	CPoint ptCurrent;
	CMenu popupMenu, *pContextMenu;
...
void CDcmImageView::SeriesPopupMenu()
{
	...
	SeriesObjectSort1Data *pSeriesSort1Obj;
	std::vector <SeriesObjectSort1Data *> lstSeriesSort1Obj;
	std::vector <SeriesObjectSort1Data *> ::iterator itSeriesSort1Obj; 
	...
	for (nIndex = 0; (nIndex < pSeriesObjectList->GetCount() && (nIndex < 40)); nIndex ++) {
		...
		if (pSeriesObject != NULL) {
			...
			pSeriesSort1Obj = new SeriesObjectSort1Data;
			pSeriesSort1Obj->index = nIndex;
			pSeriesSort1Obj->key = CStringA(sKey);
			pSeriesSort1Obj->caption = CStringA(sCaption);

			lstSeriesSort1Obj.push_back(pSeriesSort1Obj);
		}
	}

	std::stable_sort(lstSeriesSort1Obj.begin(), lstSeriesSort1Obj.end(), SeriesObjectSort1Data_Compare);

	for (itSeriesSort1Obj = lstSeriesSort1Obj.begin(); itSeriesSort1Obj != lstSeriesSort1Obj.end(); itSeriesSort1Obj ++) {
		menu.AppendMenu(MF_STRING, m_nSeriesGoIDs[(*itSeriesSort1Obj)->index], CString((*itSeriesSort1Obj)->caption.c_str()));
		delete (*itSeriesSort1Obj);
	}

	lstSeriesSort1Obj.clear();
...
void CDcmImageView::SeriesGo(UINT nID)
{
	int	ni, nIndex = -1;
	POSITION pos;
	CObList *pSeriesObjectList;
...
void CDcmImageView::SetFakeColor(UINT nID)
{
	int nColorSetNo = 0;
...
void CDcmImageView::SendImage2RIS()
{
	CMainFrame *pMainFrame = (CMainFrame *) AfxGetMainWnd();
	HWND hCallerhWnd = NULL;
	HANDLE hDIB;
	CDicomObject *pDicomObject;
	int nWidth,nHeight,nResult,nSize;
	CDC memdc;
	CBitmap membmp;
	CPalette pal;
	CCellWnd *pCellWnd;
	...
	hCallerhWnd = pMainFrame->GetCallerhWnd();

	if (!::IsWindow(hCallerhWnd)) return;
	...
	nWidth = pDicomObject->GetImageWidth();
	nHeight = pDicomObject->GetImageHeight();

	pCellWnd->ComputeExportImageSize(nWidth,nHeight);
	...
	pCellWnd->DrawObjectForExport(&memdc,nWidth,nHeight);

	hDIB = DDBToDIB(&membmp,BI_RGB,NULL);	//转化DDB2DIB
	...
	if (nSize > 0)
	{
		...
		sMapName.Format(_T("MAP3688%u"),dwTickCount);

		hMapFile = CreateFileMapping(INVALID_HANDLE_VALUE, 0, 
			PAGE_READWRITE, 0, nSize, sMapName);

		if (hMapFile != INVALID_HANDLE_VALUE)
		{
			pBuf1 = MapViewOfFile(hMapFile, FILE_MAP_ALL_ACCESS, 0, 0, nSize);
	
			if (pBuf1 != NULL)	
			{
				pBuf = ::GlobalLock(hDIB);
				memcpy(pBuf1,pBuf,nSize);
				GlobalUnlock(hDIB);
			}

			UnmapViewOfFile(pBuf1);

			nResult = ::SendMessage(hCallerhWnd,0x3688,(DWORD)dwTickCount,nSize);
			
			CloseHandle(hMapFile);

			if (nResult != 0)
				AfxMessageBox(_T("影像已成功发送,但报告系统接收失败!"),MB_ICONEXCLAMATION | MB_OK);
		}		
	}
...
HANDLE CDcmImageView::DDBToDIB(CBitmap *pBitmap, DWORD dwCompression, CPalette *pPal)
{
	BITMAP			bm;
	BITMAPINFOHEADER	bi;
...
BOOL CDcmImageView::SaveBitmap(CBitmap *pBitmap,LPCTSTR lpszPathName)   //把CBitmap类型的m_Bitmap保存到文件
{
	HANDLE hDIB;
	
	hDIB = DDBToDIB(pBitmap,BI_RGB,NULL);	//转化DDB2DIB
...
void CDcmImageView::ExportImages()
{
...
void CDcmImageView::DcmSeriesCompare()
{
	CCloudERVApp *pApp = (CCloudERVApp *) AfxGetApp();
	...
	pSeriesObject = pSelectedWnd->GetSeriesObject();
	if (pSeriesObject) {
		pApp->DcmSeriesCompare(pSeriesObject, false, nSeriesCount);
		if (nSeriesCount < 2)
...
void CDcmImageView::OnInitialUpdate()
{
...
void CDcmImageView::OnUpdate(CView* pSender, LPARAM lHint, CObject* /*pHint*/)
{
	...
	if (lHint == 1 && !m_wndDicomViewer.GetSeriesMode()) {
...
void CDcmImageView::OnUpdateActions(CCmdUI *pCmdUI)
{
	CCloudERVApp *pApp = (CCloudERVApp *) AfxGetApp();
	...
	case ID_DCMVIEW_SENDIMG2RIS:
		...
		if (bEnabled) {
			bEnabled = (hCallerhWnd != NULL);
			if (bEnabled)
				bEnabled = ::IsWindow(hCallerhWnd);
		}
	...
	case ID_DCMVIEW_EXPORT:
		...
		break;
	case ID_DCMIMAGE_COMPARE:
		bEmpty = (nCount == 0);
		if (!bEmpty) {
			pSelectedWnd = m_wndDicomViewer.GetCellWndSelected();
			if (pSelectedWnd != NULL)  {
				pSeriesObject = pSelectedWnd->GetSeriesObject();
				bEnabled = (pSeriesObject != NULL);
			}
			else 
				bEnabled = TRUE;
		}
...
void CDcmImageView::OnExecuteActions(UINT nCommandID)
{
	CCloudERVApp *pApp = (CCloudERVApp *) AfxGetApp();
